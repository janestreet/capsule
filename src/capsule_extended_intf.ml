...
module type Capsule = sig
  ...
  module Isolated : sig
    ...
    val box : ('a t[@mode u]) -> ('a boxed[@mode u])
    val unbox : ('a boxed[@mode u]) -> ('a t[@mode u])
